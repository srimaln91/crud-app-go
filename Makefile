...
build: ## Build the binary
	...
	@CGO_ENABLED=1 GOOS=$(OS) GOARCH=$(ARCH) $(GOBUILD) -ldflags \
	...
	-o $(BIN_FILE) -v ./cmd/api/main.go

.PHONY: image
image: ## Build Docker image
	@docker build --build-arg DATE=$(DATE) --build-arg COMMIT=$(COMMIT) --build-arg VERSION=$(VERSION) -t $(BINARY_NAME):$(VERSION) .
